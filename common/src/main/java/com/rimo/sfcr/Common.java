	...
	public static void init() {
		...
		PlayerEvent.PLAYER_JOIN.register(player -> {
			...
			if (! CONFIG.isEnableServer() && server != null && ! server.isHost(player.getGameProfile()))
				return;
			long seed = player.getWorld().getSeed() >> 5 & 0x7FFFFFFFFFFFFFFFL;  // don't send actually seed for anti-cheat
			...
			sendDimensionPacket(player, player.getWorld().getRegistryKey());
		});
