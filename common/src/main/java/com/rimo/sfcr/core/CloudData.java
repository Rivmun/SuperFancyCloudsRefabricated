		...
		try {
			...
			for (int cx = 0; cx < width; cx++) {
				for (int cz = 0; cz < width; cz++) {
					...
					double px = (startX - width / 2f + cx + 0.5f) * CONFIG.getCloudBlockSize();		// transform cloudpos to blockpos
					double pz = (startZ - width / 2f + cz + 0.5f) * CONFIG.getCloudBlockSize();

					// calculating density...
					if (CONFIG.isEnableWeatherDensity() && CONFIG.isBiomeDensityByChunk()) {
						if (CONFIG.isBiomeDensityUseLoadedChunk()) {
							Vec3f vec = new Vec3f(cx - width / 2f, cz - width / 2f, 0);		// stepping pos near towards to player
							vec.normalize();
							double px2 = px;
							double pz2 = pz;
