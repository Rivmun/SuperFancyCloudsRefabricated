			...
			for (int cz = 0; cz < width; cz++) {
				...
				if (isEnableDynamic && isBiomeByChunk) {
					...
					if (isBiomeUseLoadedChunk) {
						Vec3f cellPos = new Vec3f(bx, bz, 0);
						Vec3f unit = new Vec3f(cellPos.getX(), cellPos.getY(), 0);
						unit.normalize();
						unit.scale(16);  //measure as chunk
						while (!world.getChunkManager().isChunkLoaded((int) cellPos.getX() / 16, (int) cellPos.getY() / 16)
								&& unit.dot(cellPos) > 0)  //end when cellPos was reversed.
							cellPos.add(- unit.getX(), - unit.getY(), 0);  // stepping pos near towards to player

						pos = new BlockPos((int) cellPos.getX(), h, (int) cellPos.getY());
					} else {
						pos = new BlockPos(bx, h, bz);
					}
					Biome biome = world.getBiome(pos);
					f = ! CONFIG.isFilterListHasBiome(biome.getCategory())
							? getCloudDensityThreshold(densityByWeather, biome.getDownfall())
