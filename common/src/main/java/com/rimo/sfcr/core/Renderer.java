...
public class Renderer {
		...
		if (!Float.isNaN(cloudHeight)) {
			...
			synchronized (this) {
				...
				if (++timeRebuild > CONFIG.getRebuildInterval()) {
					timeRebuild = 0;
					BufferBuilder cb = rebuildCloudMesh(Tessellator.getInstance().getBuffer());
	...
	// Building mesh
	@Nullable
	private BufferBuilder rebuildCloudMesh(BufferBuilder builder) {
		...
		double fovCos = Math.cos(client.options.fov * Math.toRadians(client.player.getFovMultiplier()) * CONFIG.getCullRadianMultiplier());
