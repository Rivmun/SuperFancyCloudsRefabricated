...
import com.rimo.sfcr.mixin.GameRendererAccessor;
...
public class Renderer {
	...
	private @Nullable BufferBuilder rebuildCloudMesh(BufferBuilder builder, Vec3d cloudColor, double offset, float cloudHeight) {
		...
		if (enableCulling) {
			...
			tanHalfFov = Math.tan(Math.toRadians(CONFIG.getCullRadianMultiplier() * client.options.fov * ((GameRendererAccessor) client.gameRenderer).getLastMovementFovMultiplier()) / 2F);
