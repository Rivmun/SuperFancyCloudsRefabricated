		...
		CommandRegistrationEvent.EVENT.register((dispatcher, access, env) -> {
			dispatcher.register(literal("sfcr")
					...
					.then(literal("statu").requires(source -> source.hasPermissionLevel(2)).executes(content -> {
						...
						content.getSource().sendFeedback(Text.of("§eSnow Area Dens.: §r" + SFCReMod.COMMON_CONFIG.getSnowDensity()), false);
						content.getSource().sendFeedback(Text.of("§eRain Area Dens.: §r" + SFCReMod.COMMON_CONFIG.getRainDensity()), false);
						content.getSource().sendFeedback(Text.of("§eOther Area Dens.:§r" + SFCReMod.COMMON_CONFIG.getNoneDensity()), false);
