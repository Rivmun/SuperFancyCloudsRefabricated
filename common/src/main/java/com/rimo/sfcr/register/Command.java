		...
		CommandRegistrationEvent.EVENT.register((dispatcher, access, env) -> {
			dispatcher.register(literal("sfcr")
					...
					.then(literal("statu").requires(source -> source.hasPermissionLevel(2)).executes(content -> {
						...
						content.getSource().sendMessage(Text.of("§eBiome Affect: §r"		+ SFCReMod.COMMON_CONFIG.getBiomeDensityMultiplier()));
					...
					.then(literal("cloud").requires(source -> source.hasPermissionLevel(2))
							...
							.then(literal("color")
									...
									.executes(content -> {
										content.getSource().sendMessage(Text.of("Cloud color is " + Integer.toHexString(SFCReMod.COMMON_CONFIG.getCloudColor())));
										return 1;
									})
							)
							.then(literal("bright")
					...
					.then(literal("biome").requires(source -> source.hasPermissionLevel(2))
							.then(literal("multipler")
									...
									.executes(content -> {
										content.getSource().sendMessage(Text.of("Biome affect percent is " + SFCReMod.COMMON_CONFIG.getBiomeDensityMultiplier()));
										return 1;
									})
							)
